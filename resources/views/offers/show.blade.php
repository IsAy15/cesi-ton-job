...
<div class="c-3 bg-3 liste-v">
    <h2>Autres offres de {{ $offer->company->name }}</h2>
    <ul>
        @foreach ($relatedOffers as $relatedOffer)
        <li>
            <h3>{{ $relatedOffer->title }}</h3>
            <p>Type de contrat: {{ $relatedOffer->type }}</p>
            <p>Ville: {{ $relatedOffer->localization }}</p>
            <div class="space">
                <p>Date de début : {{ $relatedOffer->starting_date }}</p>
                <p>Date de fin : {{ $relatedOffer->ending_date }}</p>
            </div>
        </li>
        @endforeach
    </ul>
